...
let go_right ruban = 
  match ruban with
  | {left= llist ; right=[]} -> {left='\000'::llist; right=[]};
...
We first replace every character of the msg with their Write to simplify their manipulation along side with the repeats.
*)
...
@ensures reunify the identicl groups of pattern_size instructions from the program (fst_pattern@snd_pattern@program), into Repeats
@raises nothing
*)
let rec match_pattern program fst_pattern snd_pattern pattern_size acc =
  match (fst_pattern,snd_pattern) with
  (*Note: The same way than in extract_pattern, we have to reverse acc when returning it, because of the way we use :: to add new elements to it.*)
  ...
  | ([Repeat (n1,i1)], [Repeat (n2,i2)]) ->
    ...
    if i1 = i2 then
      match_pattern remaining_program [Repeat (n1+n2,i1)] new_pattern pattern_size acc
    else
      match_pattern remaining_program snd_pattern new_pattern pattern_size ((Repeat (n1,i1))::acc);
  | ([Repeat (n,i1)],i2) ->
    (*
    ...
    *)
    let (new_pattern,remaining_program) = extract_pattern program pattern_size in 
    if i1 = i2 then
      match_pattern remaining_program [Repeat (n+1,i1)] new_pattern pattern_size acc
    else
      match_pattern remaining_program snd_pattern new_pattern pattern_size ((Repeat (n,i1))::acc);
  | (instructions1,instructions2) ->
    if instructions1 = instructions2 then
      let (new_pattern,remaining_program) = extract_pattern program pattern_size in
      match_pattern remaining_program [Repeat (2,instructions1)] new_pattern pattern_size acc
    else
      match program with
...
@ensures reunify the identical groups of instrcutions, with a size of at least pattern_size, from program into Repeats
@raises nothing
*)
...
let generate_program msg = 
  let program = list_of_write msg in(*We replace every character with some write*)
  let simplified_program = simplify program 1 in(*We combine theim in repeats*)
...
let print_list l =
  print_string "[" ;
  List.iter (fun elmt -> Printf.printf "%c;" elmt) l ;
  print_string "]";;
...
print_string "MOVE TEST:\n";;
let ruban_init = {left=['b';'a'];right=['c']};;
let our_ruban = go_right ruban_init;;
let expected_ruban = {left=['c';'b';'a'];right=[]};;

if our_ruban <> expected_ruban then
  (print_string "\ntest 1 FAILURE\n";
  print_string "expected ruban: ";
  print_ruban expected_ruban;
  print_string "\nour ruban: ";
  print_ruban our_ruban;
  print_string "\n";
  exit 1)
else
  print_string "";;

let our_ruban = go_right our_ruban ;;
let expected_ruban = {left=['\000';'c';'b';'a'];right=[]} ;;
...
let our_ruban = go_left (go_left ruban_init);;
let expected_ruban = {left=[];right=['a';'b';'c']};;
...
let our_ruban = go_left expected_ruban;;
let expected_ruban = {left=[];right=['\000';'a';'b';'c']};;
...
print_string "\nWRITE TEST:\n";;
let ruban_init = {left=[];right=[]};;
let our_ruban = write 'a' ruban_init;;
let expected_ruban = {left=[];right=['a']};;
...
let ruban_init = {left=['a'];right=[]};;
let our_ruban = write 'b' ruban_init;;
let expected_ruban = {left=['a'];right=['b']};;
...
let our_ruban = write 'c' expected_ruban;;
let expected_ruban = {left=['a'];right=['c']};;
...
print_string "\nDELETE TEST:\n";;
let ruban_init = {left=['a';'b';'c';'a';'d';'a'];right=['a';'d';'e';'a';'b';'a']};;
let our_ruban = delete 'a' ruban_init;;
let expected_ruban = {left=['b';'c';'d'];right=['d';'e';'b']};;
...
print_string "\nCAESAR TEST:\n";;
let ruban_init = {left=['a';'b';'c';'d';'z'];right=['A';'B';'C';'D';'Z']};;
let our_ruban = caesar 12 ruban_init;;
let expected_ruban = {left=['m';'n';'o';'p';'l'];right=['M';'N';'O';'P';'L']};;
...
let ruban_init = {left=['a';'b';'c';'d';'z'];right=['A';'B';'C';'D';'Z']};;
let our_ruban = caesar 38 ruban_init;;
let expected_ruban = {left=['m';'n';'o';'p';'l'];right=['M';'N';'O';'P';'L']};;
...
print_string "\nINVERT TEST:\n";;
let ruban_init = {left=['b';'a'];right=['c';'d']};;
let our_ruban = invert ruban_init;;
let expected_ruban = {left=['d'];right=['c';'b';'a']};;
...
let ruban_init = {left=['a'];right=[]};;
let our_ruban = invert ruban_init;;
let expected_ruban = {left=[];right=['\000';'a']};;
...
(*EXECUTE ON PROGRAM TEST*)

print_string "\nEXECUTE ON PROGRAM TEST:\n";;

let program_init = [Write 'a'; Right; Write 'b'; Right];;
let our_ruban = execute_program program_init
let expected_ruban = {left=['b';'a'];right=[]};;
...
let program_init = [Write 'b';Left;Caesar 2;Write 'a'];;
let our_ruban = execute_program program_init
let expected_ruban = {left=[];right=['a';'d']};;
...
let program_init = [Write 'a';Right;Write 'b';Right;Write 'a'; Right; Delete 'a'];;
let our_ruban = execute_program program_init
let expected_ruban = {left=['b'];right=[]};;
...
let program_init = [Write 'a';Right;Write 'b';Right;Write 'c';Invert];;
let our_ruban = execute_program program_init
let expected_ruban = {left=[];right=['c';'b';'a']};;
...
let program_init = [Repeat (2,[Write 'a'; Right])];;
let our_ruban = execute_program program_init
let expected_ruban = {left=['a';'a'];right=[]};;
...
let program_init = [Repeat (2,[Write 'a'; Right;Repeat(3,[Write 'b';Right])])];;
let our_ruban = execute_program program_init
let expected_ruban = {left=['b';'b';'b';'a';'b';'b';'b';'a'];right=[]};;
...
let program_init = [Repeat (1,[Write 'a';Right]);Repeat (0,[Write 'b';Right])];;
let our_ruban = execute_program program_init
let expected_ruban = {left=['a'];right=[]};;
